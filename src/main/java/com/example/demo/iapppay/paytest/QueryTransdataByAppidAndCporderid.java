...
public class QueryTransdataByAppidAndCporderid {
	...
	/*
	 * 此demo 代码 使用于 cp 通过主动查询方式 获取同步数据。 请求地址见文档 请求方式:post
	 * 流程：cp服务端组装请求参数并对参数签名，以post方式提交请求并获得响应数据，处理得到的响应数据，调用验签函数对数据验签。
	 * 请求参数及请求参数格式：transdata={"appid":"123456","cporderid":"3213213"}&sign=xxxxxx&
	 * signtype=RSA 注意：只有在客户端支付成功的订单，主动查询才会有交易数据。 以下实现 各项请求参数 处理代码：
	 * 
	 */
	...
	public static void CheckSign(String appid, String cporderid) {
		...
		System.out.println(respData+" asd");

		/*---------------------------------------------如果得到成功响应的结果-----------------------------------------------------------*/
		// 解析结果 得到的 数据为一个以&分割的字符串，需要分成三个部分transdata，sign，signtype。
		// 成功示例：respData ==
		// "transdata={"appid":"500000185","cporderid":"1421220957285","cpprivate":"cpprivateinfo123456","currency":"RMB","feetype":6,"money":1.0,"paytype":5,"result":0,"transid":"32011501141535580250","transtime":"2015-01-14
		// 15:36:23","waresid":4}&sign=cbKD9RDjYpjgPMaaWJ0K4q7f6ldXOj8QA+IWkPdCfjmOlj2Gb9pRQg88CAONSB49wsNvHkiM+4OAOpqcohN/kdgDc92fmqzGcPb5GyUSdDacKHWFqFkyxxeboFiH0P5SrDIsklHBAfBiWP5DXTnL1Y6RiG52JyjX2sRWnGm1dAI=&signtype=RSA"
		...
		if (signtype == null) {
			...
			try {
				...
				System.out.println(transdata.equals(reslutMap.get("transdata")));
			...
			try {
				...
				System.out.println(sign.equals(reslutMap.get("sign")));
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
	...
	public static void main(String[] args) {
		try {
			System.out.println(URLDecoder.decode("asdqwe+qe", "UTF-8"));
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		CheckSign("3019332996", "12345678");
